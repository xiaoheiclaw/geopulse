...
⚠ 局限性（红队审计 2026-03-07 发现）：
1. implied_prob_from_option 输出的是 Q-measure（风险中性概率），
   不是 P-measure（真实世界概率）。尾部事件中两者差 2-3 倍。
   不可直接和 DAG 的真实世界概率对比。
2. reference_class_adjust 的调整因子是主观赋值。
   参考类样本量通常 <5，置信区间极宽。
3. decompose_prob 的子概率本身也是估计。
   误差会乘法传播。

正确用法：作为 sanity check（"我的判断偏离市场/历史多远？"），
不是作为 calibration target（"应该改成这个数字"）。
"""
